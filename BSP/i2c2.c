...
void I2C2_Configuration(void)
{
  ...
  /* Configure I2C2 pins: PB10->I2C1_SCL and PB11->I2C1_SDA */
  RCC_APB2PeriphClockCmd(I2C2_SCL_GPIO_RCC, ENABLE);
...
{	
  ...
  * @retval 无
  */
static FunctionalState I2C2_Start(void)
{
	SDA2_H;
	SCL2_H;
	I2C_delay();
...
{	
  ...
  */
static FunctionalState I2C2_WaitAck(void) 	
{
	SCL2_L;
	I2C_delay();
	SDA2_H;			
...
{				  
	  u8 temp=0;		  	    																 
    I2C2_Start();  
	  I2C2_SendByte(0x08<<1);   //发送器件地址0XA0,写数据 	 
	  I2C2_WaitAck();
    I2C2_SendByte(ReadAddr);   //发送低地址
		I2C2_WaitAck();    
	  I2C2_Start();  	 	   
	  I2C2_SendByte(0X11);           //进入接收模式			   
	  I2C2_WaitAck(); 
    temp=I2C2_ReceiveByte();		   
    I2C2_Stop();//产生一个停止条件	 
    return temp;
...
{				   	  	    																 
    I2C2_Start();  
		...
		I2C2_SendByte(0X10+((WriteAddr/256)<<1));   //发送器件地址0XA0,写数据 
		 
	 I2C2_WaitAck();	   
    I2C2_SendByte(WriteAddr%256);   //发送低地址
	I2C2_WaitAck(); 										  		   
	I2C2_SendByte(DataToWrite);     //发送字节							   
I2C2_WaitAck();	    	   
   I2C2_Stop();//产生一个停止条件 
	delay_ms(10);	 
}
void IIC2_write_one_byte_CRC(u16 WriteAddr,u16 DataToWrite)
{
  ...
  DataBuffer[0] = 0X08 << 1;
    ...
    I2C2_Start();  	
		I2C2_SendByte( DataBuffer[0]);   //发送器件地址0XA0,写数据 		 
	  I2C2_WaitAck();	   
    I2C2_SendByte(DataBuffer[1]);   //发送地址
	  I2C2_WaitAck(); 										  		   
	  I2C2_SendByte(DataBuffer[2]);     //发送字节							   
    I2C2_WaitAck();	
	  I2C2_SendByte(DataBuffer[3]);     //发送CRC
	  I2C2_WaitAck();
    I2C2_Stop();//产生一个停止条件 	  
	  delay_ms(10);
}
...
{		
    if(!I2C2_Start())return DISABLE;
    I2C2_SendByte((((WriteAddress & 0x0700) >>7) | DeviceAddress) & 0xFFFE); /*设置高起始地址+器件地址 */
    if(!I2C2_WaitAck()){I2C2_Stop(); return DISABLE;}
    I2C2_SendByte((uint8_t)(WriteAddress & 0x00FF));   /* 设置低起始地址 */      
    I2C2_WaitAck();	
...
{		
    if(!I2C2_Start())return DISABLE;
    I2C2_SendByte((((ReadAddress & 0x0700) >>7) | DeviceAddress) & 0xFFFE); /* 设置高起始地址+器件地址 */ 
    if(!I2C2_WaitAck()){I2C2_Stop(); return DISABLE;}
    I2C2_SendByte((uint8_t)(ReadAddress & 0x00FF));   /* 设置低起始地址 */      
    I2C2_WaitAck();
    I2C2_Start();
